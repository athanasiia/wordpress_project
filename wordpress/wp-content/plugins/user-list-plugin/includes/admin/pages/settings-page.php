    ...
    if (isset($_POST['ulp_save_settings']) && check_admin_referer('ulp_settings_nonce')) {
        ...
        if (!is_numeric($days)) {
            ...
            if (!empty($token) && !ulp_encrypt_and_save_token($token)) {
                echo '<div class="notice notice-error"> Could not update token </div>';
            }
        ...
        <form method="post">
            ...
            <table class="form-table">
                ...
                <tr>
                    ...
                    <td>
                        <input type="password" name="ulp_gorest_token" class="regular-text" value="" placeholder="Enter your GoREST API token"/>
                        <?php if (ulp_get_decrypted_token()): ?>
                            <p class="description"> Token is currently set. Enter new token to replace it.</p>
                        <?php endif; ?>
                    </td>
                </tr>
                <tr>
